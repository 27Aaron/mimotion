...
Xiaomi/Zepp 自动刷步服务。用户绑定小米账号后，通过 cron 定时随机写入步数，支持 Bark 推送通知。
...
- **Notifications**: Bark push service
...
app/
  ...
  api/
    ...
    cron/                   # 启动调度器
lib/
  db/schema.ts              # Drizzle schema (users, xiaomi_accounts, schedules, run_logs, invite_codes)
  db/index.ts               # 数据库连接
  auth.ts                   # JWT + 密码工具
