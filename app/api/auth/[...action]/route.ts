...
async function handleRegister(request: NextRequest) {
  ...
  if (typeof username !== 'string' || username.length < 2 || username.length > 32) {
    return NextResponse.json({ error: '用户名长度需在 2-32 之间' }, { status: 400 })
  }
  ...
  try {
    ...
    if (msg === 'INVALID_CODE') return NextResponse.json({ error: '邀请码无效' }, { status: 400 })
    if (msg === 'CODE_USED') return NextResponse.json({ error: '邀请码已使用' }, { status: 400 })
    if (msg === 'USERNAME_TAKEN') return NextResponse.json({ error: '用户名已被注册' }, { status: 400 })
